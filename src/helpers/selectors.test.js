...
const state = {
  ...
  appointments: {
    ...
    "4": { 
      id: 4, 
      time: "3pm", 
      interview: { student: "Bill", interviewer: 3} 
    },
    "5": {
      id: 5,
      time: "4pm",
...
test("getInterviewersForDay returns an array containing the correct appointment objects", () => {
  ...
  expect(first).toEqual(state.appointments["4"].interview.interviewer);
  expect(second).toEqual(state.appointments["5"].interview.interviewer);
